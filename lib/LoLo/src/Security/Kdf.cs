...
public static class Kdf
{
   ...
   public static bool VerifyOld(string password, string kdfHashedPassword, bool isPwBase64PrehashedSha512 = false)
   {
      if (isPwBase64PrehashedSha512 is false)
      {
         ...
         var hashBytes = System.Security.Cryptography.SHA512.HashData(bytes);
   ...
   private static void _GetPwBytes(string password, bool isPwBase64PrehashedSha512, out byte[] pwBase64EncodedSha512Bytes)//, out string hash4)
   {
      ...
      if (isPwBase64PrehashedSha512)
      {
         ...
         var bytes = password._ToBytes();
         var pwSha512Bytes = System.Security.Cryptography.SHA512.HashData(bytes);
   ...
   public class Tests
   {
      ...
      internal async Task LegacyVerify()
      {
         ...
         var result = await Kdf.Verify(pw, argon2OutputDigest);
         var result2 = Kdf.VerifyOld(pw, argon2OutputDigest);
      ...
      internal async Task BasicHash()
      {
         ...
         var digest = Kdf.Hash(pw);
         var result =await Kdf.Verify(pw, digest);         
         __.Assert(result);

         var result2 =await  Kdf.Verify(pw, "$argon2id$v=19$m=65536,t=3,p=1$wj+z7XEcaMUPZpA8/KNj5w$C3g2qHxgXH52x2ndoYcq0LtkyiGJdPBWNTb8riXM9y0");
         __.Assert(result2);
